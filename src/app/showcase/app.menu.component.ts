    ...
    template: `
        <div class="layout-sidebar" [ngClass]="{'active': active}">
            ...
            <p-divider align="right">
                <button pButton pRipple type="button" (click)="show()" icon="pi pi-cog" class="p-button-rounded p-button-text"></button>
            </p-divider>
            ...
            <div class="layout-menu">
                ...
                <div *ngFor="let item of model; let i = index;">
                    ...
                    <div *ngFor="let subitem of item.items;" class="menu-items">
                        <a *ngIf="subitem.level == 1" [routerLink]="subitem.routerLink" routerLinkActive="router-link-exact-active">{{subitem.label}}</a>                        
                        <div *ngIf="subitem.level == 2">
                            <a tabindex="0" (click)="toggleSubmenu($event, subitem.routerLink)"><i class="pi pi-plus"></i>{{subitem.label}}</a>
                            <div [@submenu]="isSubmenuActive(subitem.routerLink) ? 'visible': 'hidden'" (@submenu.done)="onAnimationDone()">
                                <ul>
                                    <li *ngFor="let subsubitem of subitem.items;" >
                                        <a [routerLink]=" [subsubitem.routerLink]" routerLinkActive="router-link-exact-active" [routerLinkActiveOptions]="{exact:true}">{{subsubitem.label}}</a>
                                    </li>                               
                                </ul>
                            </div>
                        </div>
...
export class AppMenuComponent implements OnInit{
    ...
    ngOnInit() {
        this.model = [
            ...
            // },
            // {
            //     label: 'Data', icon: 'pi pi-fw pi-star', routerLink: ['/uikit'],
            //     items: [
            //         {label: 'dataview', icon: 'pi pi-fw pi-check-square', routerLink: ['/dataview'], badge: '6', badgeClass: 'p-badge-danger'},
            //         {label: 'fullcalendar', icon: 'pi pi-fw pi-bookmark', routerLink: ['/fullcalendar']},
            //         {label: 'orderlist', icon: 'pi pi-fw pi-exclamation-circle', routerLink: ['/orderlist']},
            ...
            // },
            {
                ...
                items: [
                    {label: 'Crud', icon: 'pi pi-fw pi-pencil', routerLink: ['/pages/crud'], level: 1},
                    {label: 'Calendar', icon: 'pi pi-fw pi-calendar-plus', routerLink: ['/pages/calendar'], level: 1},
                    {label: 'Timeline', icon: 'pi pi-fw pi-calendar', routerLink: ['/pages/timeline'], level: 1},
                    {
                        label: 'Table', icon: 'pi pi-fw pi-globe', badge: '2', badgeClass: 'p-badge-warning',
                        items: [
                            {label: 'basic', icon: 'pi pi-fw pi-globe', routerLink: '/table/basic', level: 1},
                            {label: 'dynamic', icon: 'pi pi-fw pi-globe', routerLink: ['/table/dynamic'], level: 1},
                            {label: 'templating', icon: 'pi pi-fw pi-globe', routerLink: ['/table/templating'], level: 1},
                            {label: 'dynamic', icon: 'pi pi-fw pi-globe', routerLink: ['/table/dynamic'], level: 1}
                        ],
                        level: 2,
                    },
                    {label: 'Login', icon: 'pi pi-fw pi-sign-in', routerLink: ['/login'], level: 1},
                    {label: 'Invoice', icon: 'pi pi-fw pi-dollar', routerLink: ['/pages/invoice'], level: 1},
                    {label: 'Help', icon: 'pi pi-fw pi-question-circle', routerLink: ['/pages/help'], level: 1},
                    {label: 'Error', icon: 'pi pi-fw pi-times-circle', routerLink: ['/error'], level: 1},
                    {label: 'Not Found', icon: 'pi pi-fw pi-exclamation-circle', routerLink: ['/notfound'], level: 1},
                    {label: 'Access Denied', icon: 'pi pi-fw pi-lock', routerLink: ['/access'], level: 1},
                    {label: 'Contact Us', icon: 'pi pi-fw pi-pencil', routerLink: ['/contactus'], level: 1},
                    {label: 'Empty', icon: 'pi pi-fw pi-circle-off', routerLink: ['/pages/empty'], level: 1}
                ],
                level : 0,
            },
